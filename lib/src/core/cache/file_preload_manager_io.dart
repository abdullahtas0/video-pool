...
import 'file_preload_types.dart';
...
class FilePreloadManager {
  ...
  Future<void> cleanupIncomplete({
    ...
    for (final key in toRemove) {
      ...
      if (cached != null) {
        ...
        _currentCacheSizeBytes = (_currentCacheSizeBytes - cached.sizeBytes)
            .clamp(0, _currentCacheSizeBytes);
      }
    }
    ...
    try {
      ...
      if (existing != null && !existing.complete) {
        ...
        if (await partialFile.exists()) {
          ...
          _currentCacheSizeBytes = (_currentCacheSizeBytes - existing.sizeBytes)
      ...
      final oldEntry = _diskCache.remove(key);
      if (oldEntry != null) {
        _currentCacheSizeBytes = (_currentCacheSizeBytes - oldEntry.sizeBytes)
            .clamp(0, maxCacheSizeBytes);
      }
  ...
  Future<void> _evictIfNeeded(int additionalBytes) async {
    while (_currentCacheSizeBytes + additionalBytes > maxCacheSizeBytes &&
      ...
      if (oldest != null) {
        _currentCacheSizeBytes = (_currentCacheSizeBytes - oldest.sizeBytes)
            .clamp(0, maxCacheSizeBytes);
  ...
  Future<void> _saveManifest() async {
    try {
      final entries = _diskCache.entries.map((e) => e.value.toJson()).toList();
      final manifestFile = File('$cacheDirectory/_manifest.json');
